...
function obtenerTiempoFaltante(fechaLimite){
    ...
    if (tiempoFaltante <= 0) { // if remaining time is less than or equal to zero, return zero values
        return {
...
function cuentaRegresiva(tiempoFaltante,mensaje) {
    ...
    const tiempoActual = setInterval( () =>{
        ...
        if(t.tiempoFaltante <= 1){
            ...
            botonPlay.classList.add("button-hover");
            botonPause.classList.add("button-hover");

        }
    }, 1000)
}

cuentaRegresiva('NOV 08 2023 22:07:00', 'Feliz Navidad')